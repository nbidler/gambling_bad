function factorial(target_number){
    ...
    else if (target_number < 2){return 1;}
    let total = 1;
    for (let loop = 2; loop <= target_number; loop++){
        total *= loop;
    }
...
function combination(total_options, selected){
    ...
    for (let loop = total_options-selected+1; loop <= total_options; loop++){
        total *= loop;
        console.log(loop, total);
    } 
    for (let loop = 1; loop <= selected; loop++){
        total /= loop;
        console.log(loop, total);
    }
...
$( document ).ready(function() {
    ...
    $( "#inputSubmit" ).click(function() {
        ...
        $("#outputWorthwhile").text(`${possibilities} * \$${lotto_cost}, in total \$${lotto_total}`);
